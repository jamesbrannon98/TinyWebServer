 ...
 */
/* $begin serve_static */
/* If the type is 0, then the function performs as it
 * originally would. Otherwise, it checks to see if
 * the conditions are met for the specified type.
 * If they are, then the function prints the range
 * specified by the client. Otherwise, a 416 is
 * returned.
 */
void serve_static(int fd, char *filename, int filesize, int size_flag, rangeNode *nodePtr) 
{
  ...
  size_t writesize;
  int type = nodePtr->type;
  int first = nodePtr->first;
  int second = nodePtr->second;
  int rangeLength = (second - first + 1);
  ...
  if(type == 0){
    ...
    if((first >= 0) && (second >= 0) && (second > first) && (filesize > first)){
      if(second > (filesize - 1)){
        second = (filesize - 1);
      }
      /* Send response headers to client */
      get_filetype(filename, filetype);       //line:netp:servestatic:getfiletype
      ...
      srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0);//line:netp:servestatic:mmap
      Close(srcfd);                           //line:netp:servestatic:close
      if (rio_writen(fd, (srcp + first), rangeLength) < rangeLength) {
        printf("errors writing to client.\n");         //line:netp:servestatic:write
      }
      Munmap(srcp, filesize);                 //line:netp:servestatic:munmap
    }
    ...
    if((first >= 0) && (filesize > first)){
      ...
      srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0);//line:netp:servestatic:mmap
      Close(srcfd);                           //line:netp:servestatic:close
      if (rio_writen(fd, (srcp + first), filesize) < filesize) {
        printf("errors writing to client.\n");         //line:netp:servestatic:write
      }
      Munmap(srcp, filesize);                 //line:netp:servestatic:munmap
    }
    ...
    if((first >= 0) && ((filesize + 1) > first)){
      ...
      sprintf(buf, "%sConnection: close\r\n", buf);
      sprintf(buf, "%sAccept-Ranges: bytes\r\n", buf);
      sprintf(buf, "%sContent-Range: bytes %d-%d/%d\r\n", buf, (filesize - first), (filesize - 1), filesize);
      if (size_flag == 1) {
        sprintf(buf, "%sContent-length: %d\r\n", buf, (filesize - (filesize - first)));
      }
      sprintf(buf, "%sContent-type: %s\r\n\r\n", buf, filetype);
      writesize = strlen(buf);
      if (rio_writen(fd, buf, strlen(buf)) < writesize) {
      ...
      srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0);//line:netp:servestatic:mmap
      Close(srcfd);                           //line:netp:servestatic:close
      if (rio_writen(fd, (srcp + (filesize - first)), filesize) < filesize) {
        printf("errors writing to client.\n");         //line:netp:servestatic:write
      }
      Munmap(srcp, filesize);                 //line:netp:servestatic:munmap
    }
